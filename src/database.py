...
class DatabaseManager:
    ...
    def get_or_create_course(self, name: str, user_id: str) -> int:
        clean = name.strip()
        result = self.sb.table("courses").select("id").eq("name", clean).eq("user_id", user_id).execute()
        if result.data:
            return result.data[0]["id"]
        insert_result = self.sb.table("courses").insert({"name": clean, "user_id": user_id}).execute()
        return insert_result.data[0]["id"]

    def add_video_data(self, data: dict, user_id: str) -> int:
        existing = (
        ...
        if existing.data:
            ...
            self.sb.table("videos").update(payload).eq("id", vid_id).eq("user_id", user_id).execute()
            return vid_id
        result = self.sb.table("videos").insert(payload).execute()
        return result.data[0]["id"]

    def get_video_info_for_quiz(self, video_db_id: int, user_id: str):
        result = (
            self.sb.table("videos")
    ...
    def update_user_notes(self, video_id: int, notes: str, user_id: str):
        self.sb.table("videos").update({"user_notes": notes}).eq("id", video_id).eq("user_id", user_id).execute()

    def get_all_courses(self, user_id: str):
        result = self.sb.table("courses").select("id, name").eq("user_id", user_id).order("name").execute()
        return [(row["id"], row["name"]) for row in result.data]

    def get_videos_for_course(self, course_id: int, user_id: str):
        result = (
            self.sb.table("videos")
            .select("id, title")
    ...
    def get_all_courses_full(self, user_id: str) -> list:
        """Single-pass fetch of all courses + videos + question counts.
        Replaces the N+1 loop in /api/courses — 3 queries instead of 1+2N+V.
        """
        # 1. All courses
        courses_res = self.sb.table("courses").select("id, name").eq("user_id", user_id).order("name").execute()
    ...
    def delete_video(self, video_id: int, user_id: str):
        try:
            self.sb.table("quiz_questions").delete().eq("video_id", video_id).eq("user_id", user_id).execute()
            self.sb.table("videos").delete().eq("id", video_id).eq("user_id", user_id).execute()
    ...
    def get_course_stats(self, course_id: int, user_id: str):
        try:
            v_result = (
                self.sb.table("videos")
    ...
    def get_video_stats(self, video_id: int, user_id: str) -> int:
        try:
            result = (
                self.sb.table("quiz_questions")
                .select("id", count="exact")
    ...
    def add_quiz_questions(self, video_db_id: int, questions: list, user_id: str) -> int:
    ...
    def get_due_review_count(self, user_id: str) -> int:
        today = datetime.now().date().isoformat()
        result = (
            self.sb.table("quiz_questions")
            .select("id", count="exact")
    ...
    def get_due_questions(self, user_id: str, limit=None):
        today = datetime.now().date().isoformat()
        query = (
            self.sb.table("quiz_questions")
    ...
    def get_questions_by_video(self, video_id: int, user_id: str, limit=None):
        query = (
            self.sb.table("quiz_questions")
            .select("id, question, options, answer")
            .eq("video_id", video_id)
    ...
    def get_questions_by_course(self, course_id: int, user_id: str, limit=None):
    ...
    def get_all_questions(self, user_id: str, limit=None):
        query = self.sb.table("quiz_questions").select("id, question, options, answer").eq("user_id", user_id)
        if limit:
            query = query.limit(limit)
        result = query.execute()
        return [(r["id"], r["question"], r["options"], r["answer"]) for r in result.data]

    def update_srs_level(self, question_id: int, performance: str, user_id: str):
        result = (
            self.sb.table("quiz_questions")
